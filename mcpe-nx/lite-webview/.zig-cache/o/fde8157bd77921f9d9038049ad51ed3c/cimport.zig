...
pub const webview_version_info_t = extern struct {
    version: webview_version_t = @import("std").mem.zeroes(webview_version_t),
    version_number: [32]u8 = @import("std").mem.zeroes([32]u8),
    pre_release: [48]u8 = @import("std").mem.zeroes([48]u8),
    build_metadata: [48]u8 = @import("std").mem.zeroes([48]u8),
};
...
pub extern fn webview_dispatch(w: webview_t, @"fn": ?*const fn (w: webview_t, arg: ?*anyopaque) callconv(.c) void, arg: ?*anyopaque) void;
...
pub extern fn webview_bind(w: webview_t, name: [*c]const u8, @"fn": ?*const fn (seq: [*c]const u8, req: [*c]const u8, arg: ?*anyopaque) callconv(.c) void, arg: ?*anyopaque) void;
...
pub const _LIBCPP_ABI_NAMESPACE = @compileError("unable to translate macro: undefined identifier `__1`"); // <command line>:3:9
...
pub const _LIBCPP_HARDENING_MODE = @compileError("unable to translate macro: undefined identifier `_LIBCPP_HARDENING_MODE_NONE`"); // <command line>:19:9
pub const WEBVIEW_HEADER = "";
pub const WEBVIEW_H = "";
pub const WEBVIEW_API = @compileError("unable to translate C expr: unexpected token 'extern'"); // src/webview.h:29:9
...
pub const WEBVIEW_VERSION_PRE_RELEASE = "";
pub const WEBVIEW_VERSION_BUILD_METADATA = "";
pub const WEBVIEW_STRINGIFY = @compileError("unable to translate C expr: unexpected token ''"); // src/webview.h:58:9
pub inline fn WEBVIEW_EXPAND_AND_STRINGIFY(x: anytype) @TypeOf(WEBVIEW_STRINGIFY(x)) {
    _ = &x;
    return WEBVIEW_STRINGIFY(x);
}
pub const WEBVIEW_VERSION_NUMBER = WEBVIEW_EXPAND_AND_STRINGIFY(WEBVIEW_VERSION_MAJOR) ++ "." ++ WEBVIEW_EXPAND_AND_STRINGIFY(WEBVIEW_VERSION_MINOR) ++ "." ++ WEBVIEW_EXPAND_AND_STRINGIFY(WEBVIEW_VERSION_PATCH);
