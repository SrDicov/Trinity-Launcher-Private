...
static void forceRemoteConnect() {
    std::string marker = PathHelper::getPrimaryDataDirectory() + "remote_enable";
    std::string path = getOptionsPath();
    FileUtil::mkdirRecursive(PathHelper::getParentDir(path));
    if(FileUtil::exists(marker)) {
        Log::info("Launcher", "Remote connect already enabled (%s), skipping", marker.c_str());
        return;
    }
    ...
    bool found = false;
    {
        ...
        while(std::getline(in, line)) {
            if(!line.empty() && line.back() == '\r')
                line.pop_back();
            if(line.rfind("xbl_use_remote_connect:", 0) == 0) {
                line = "xbl_use_remote_connect:1";
                found = true;
            }
    ...
    if(!found)
        lines.emplace_back("xbl_use_remote_connect:1");
    std::ofstream out(path);
    if(!out.is_open()) {
        Log::error("Launcher", "Failed to force xbl_use_remote_connect=1, could not open %s", path.c_str());
        return;
    }
    ...
    if(markerFile.is_open())
        markerFile.close();
    Log::info("Launcher", "Forced xbl_use_remote_connect=1 in game options (%s), one time marker created (%s)", path.c_str(), marker.c_str());
}
